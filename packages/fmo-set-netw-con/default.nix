...
writeShellApplication {
  ...
  text = ''
    ...
    set_network_con() {
        ...
        #NCON=$(DBUS_SYSTEM_BUS_ADDRESS=unix:path=/tmp/dbusproxy_net.sock nmcli con show --active|cut -d" "  -f1|fzf)
        NCON=$(nmcli con show |cut -d" "  -f1|fzf)
        ...
        sudo nmcli con mod "$NCON" ipv4.addresses "$CON_IP"/"$NETMASK" \
            ipv4.gateway "$GW_IP" \
            ipv4.dns 8.8.8.8 \
            ipv4.method manual
    ...
    [yY][eE][sS] | [yY])
        ...
        ;;
    *)
        echo "Set network connection manually"
        ;;
    esac
