...
{ inputs, ... }:
{
  flake.overlays.own-pkgs-overlay = final: _prev: {
    ...
    inherit (inputs.device-assembly-toolset.packages.${final.stdenv.hostPlatform.system})
      device-assembly-agent
      ;
    kms-enrolment = inputs.kms-enrolment.packages.${final.stdenv.hostPlatform.system}.default;
  };
}
